...
const AnimatedChatDemo = ({ isActive }: { isActive: boolean }) => {
  const [messages, setMessages] = useState([
    { text: "Hola! En que puedo ayudarte hoy?", isBot: true, visible: false },
    { text: "Me gustaria reservar una cita", isBot: false, visible: false },
    { text: "Perfecto! Puedo ayudarte con eso. Que servicio te interesa?", isBot: true, visible: false },
  ])
  ...
  useEffect(() => {
    ...
    const scenarios = [
      [
        { text: "Hola! En que puedo ayudarte hoy?", isBot: true },
        { text: "Me gustaria reservar una cita", isBot: false },
        { text: "Perfecto! Puedo ayudarte con eso. Que servicio te interesa?", isBot: true },
      ],
      [
        { text: "Hola! Estoy disponible 24/7 para asistirte.", isBot: true },
        { text: "Teneis disponibilidad los fines de semana?", isBot: false },
        { text: "Puedo consultar nuestras franjas del fin de semana.", isBot: true },
      ],
      [
      ...
      <div className="relative">
        ...
        {callState === "answered" && (
          <div className="absolute -bottom-2 left-1/2 transform -translate-x-1/2">
            <div className="bg-blue-100 px-2 py-1 rounded text-xs text-blue-700 whitespace-nowrap">Llamada respondida</div>
          </div>
        )}
      </div>
    </div>
...
const AnimatedCalendarDemo = ({ isActive }: { isActive: boolean }) => {
  ...
  return (
    <div className="bg-slate-50 rounded-lg p-4 h-32">
      ...
      {booked && (
        <div className="mt-2 text-xs text-green-600 font-medium animate-fade-in">{"✓ Cita reservada para el dia 15"}</div>
      )}
    </div>
  )
}
...
const AnimatedEmailDemo = ({ isActive }: { isActive: boolean }) => {
  const [emails, setEmails] = useState([
    ...
    { subject: "Presupuesto necesario", status: "unread" },
  ])
